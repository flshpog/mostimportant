...
module.exports = {
    data: (() => {
        const builder = new SlashCommandBuilder()
            .setName('mute1on1s')
            .setDescription('Deny view/speak for involved player roles across one or more categories');

        for (let i = 1; i <= 10; i++) {
            builder.addChannelOption(option =>
                option.setName(i === 1 ? 'category' : `category${i}`)
                    .setDescription(i === 1 ? 'The category whose 1-1 channels to mute' : `Additional category ${i} (optional)`)
